"""
🏋️ Тренировочный цикл для 1B модели
...
BATCH_SIZE = 2             # Микро-батч (сколько примеров за один forward pass)
                           # ⚠️  FP8 без gradient checkpointing требует больше VRAM для активаций.
                           #     На RTX 5090 (32GB): batch=8 и batch=4 вызывают OOM, batch=2 стабилен.
GRADIENT_ACCUM_STEPS = 64  # Кол-во микро-батчей перед обновлением весов
                           # Эффективный батч = BATCH_SIZE × GRADIENT_ACCUM_STEPS × block_size
                           # = 2 × 64 × 2048 = 262,144 токенов ≈ 0.26M (тот же effective batch)

# --- Learning Rate ---
LEARNING_RATE = 1e-4       # Пиковый LR (более стабильный для 1B, Llama-style)
MIN_LR = 1e-5             # Минимальный LR (10% от пикового)
WARMUP_STEPS = 2000        # Разогрев (линейно растём от 0 до peak LR)
LR_DECAY_STEPS = 100_000   # Полный цикл cosine decay
...
RESTART_T0 = 20_000        # Длина первого цикла (шаги)
RESTART_T_MULT = 2         # Каждый следующий цикл в T_MULT раз длиннее
                           # Циклы: 20K → 40K → 80K (итого: 140K шагов покрывают 100K)
...
def apply_test_config():
    ...
    BATCH_SIZE = 4             # Меньше батч — меньше VRAM
    GRADIENT_ACCUM_STEPS = 4   # Effective batch = 4 × 4 × 2048 = 32K токенов
...
def train():
    ...
    start_step = 0
    resume_path = CHECKPOINT_DIR / "klenskiy-gpt-1b-best.pt"
    if not resume_path.exists():
        # Ищем последний step_*
        steps = sorted(CHECKPOINT_DIR.glob("klenskiy-gpt-1b-step_*.pt"))
        if steps:
            resume_path = steps[-1]
        else:
            resume_path = None
...
def save_checkpoint(model, optimizer, config, tokenizer, step, name):
    """Сохраняет чекпоинт."""
    path = CHECKPOINT_DIR / f"klenskiy-gpt-1b-{name}.pt"
...
if __name__ == "__main__":
    if "--test" in sys.argv:
        ...
        print("🧪  Шагов: 500, Batch: 4×4×2048 = 32K tok/step")
