...
class ModelConfig:
    block_size: int = 2048     # Длина контекста для обучения (2048 — стандарт для 1B моделей)

    n_embd: int = 2048         # Размер эмбеддинга (hidden dimension)

    n_head: int = 16           # Количество Query-голов
    n_kv_head: int = 4         # Количество KV-голов (для GQA)
                               # n_head / n_kv_head = 4 → каждая KV-голова обслуживает 4 Q-головы

    n_layer: int = 22          # Количество трансформер-блоков
    ...
    # None = без scaling (стандартный 2048)
    # {"type": "ntk", "factor": 2.0} = NTK-aware, контекст × factor (4096)
    # {"type": "ntk", "factor": 4.0} = NTK-aware, контекст × factor (8192)
    rope_scaling: dict = None

    # Gradient checkpointing — пересчитываем активации вместо хранения
    # Экономит ~60% VRAM ценой ~30% скорости. Включай если не хватает памяти.
    use_gradient_checkpointing: bool = False

    # Precision mode для обучения:
    # "bf16"  — стандартный mixed precision (все GPU)
    # "fp8"   — MXFP8 через Transformer Engine (Hopper H100+, Blackwell)
    # "fp4"   — NVFP4 через Transformer Engine (только Blackwell B200/B100)
    #
    # FP8: ~2x быстрее bf16, те же параметры памяти
    # FP4: ~4x быстрее bf16, ~2x меньше VRAM для весов
    #
    # Для FP4 последние N слоёв работают в FP8 (они чувствительнее к квантизации)
    precision_mode: str = "bf16"
    fp4_high_precision_layers: int = 2  # Последние N слоёв в FP8 при FP4 режиме

    # Формула для ~1B параметров:
    # Embedding:  vocab_size × n_embd = 32K × 2048 = 65M
    # Per layer:  ~4 × n_embd² + 3 × n_embd × intermediate = ~40M
    # 22 layers:  22 × 40M ≈ 880M
    # Total:      ~945M ≈ 1B
...
class GroupedQueryAttention(nn.Module):
    def __init__(self, config, use_te=False):
        ...
        self.n_head = config.n_head         # 16 Q-голов
        self.n_kv_head = config.n_kv_head   # 4 KV-головы
        self.n_rep = self.n_head // self.n_kv_head  # 4 — коэффициент повторения
...
class SwiGLUFeedForward(nn.Module):
    def __init__(self, config, use_te=False):
        super().__init__()
        hidden = config.intermediate_size  # 5632 для n_embd=2048
